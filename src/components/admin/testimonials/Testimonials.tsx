...
const Testimonials = () => {
 ...
 return Swal.fire({
  ...
  return <div>
    <PageTitle title='відгуки' isAddButtonDisplayed={true} isSettingsButtonDisplayed={true} text='Додати відгук' href='/testimonials/add'/>
    
<ul className={css.testimonials__list}>
  {testimonials?.map(({id, message, name}) => <li key={id} className={css.testimonials__item}>
...
<Link href={`/admin/testimonials/edit/${id}`} onClick={()=> console.log(id)} className={css.editBtn}>
  <EditIcon />
 </Link>   
</div>
  </li>)}
